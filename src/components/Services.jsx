...
export default function Services() {
    ...
    return (
        <div className="min-h-4/5 h-auto p-6 bg-gradient-to-br from-gray-300 via-slate-200 to-gray-300">
            <motion.h1
            ...
            <div
                className="flex justify-center gap-x-3 gap-y-4 flex-wrap items-center"
            >
                {servicesData.map((item, index) => {
                    ...
                    return (
                        <motion.div
                            key={index}
                            className="border-2 border-[#E85C0D] rounded-xl min-h-[300px] h-auto w-[280px] bg-[#F6F7C4] p-4 flex flex-col gap-y-3 overflow-hidden cursor-pointer shadow-lg"
                            initial={{ opacity: 0, x: -20 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ type: "spring", stiffness: 200, delay: 0.2 * index }}
                            whileHover={{ scale: 1.05 }}
                        >
                            <div className="text-6xl text-[#D91656]">
                                {item.icon}
                            </div>
                            <motion.h2
                                initial={{ opacity: 0, x: -10 }}
                                animate={{ opacity: 1, x: 0 }}
                                transition={{ type: "spring", stiffness: 200, delay: 0.4 * index }}
                                className="text-xl text-[#5A6C57] font-bold"
                            >
                                {item.title}
                            </motion.h2>
                            <motion.p
                                className={`text-justify text-[#31511E] font-sans transition-all ${isExpanded ? "line-clamp-none" : "line-clamp-3"
                                    }`}
                                initial={{ opacity: 0, y: -10 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ type: "spring", stiffness: 200, delay: 0.5 * index }}
                            >
                                {item.desc}
                            </motion.p>
                            <button
